...
export class BackupRestoreService {
    ...
    private collectUnifiedData(): any {
        ...
        if (appState) {
            ...
            // Transactions
            if (appState.transactions && appState.transactions.entities) {
                const transactions = Object.values(appState.transactions.entities);
                if (transactions.length > 0) {
                    data[LocalStorageKey.GUEST_TRANSACTIONS] = transactions;
                }
            }

            // Accounts
            if (appState.accounts && appState.accounts.entities) {
                const accounts = Object.values(appState.accounts.entities);
                if (accounts.length > 0) {
                    data[LocalStorageKey.GUEST_ACCOUNTS] = accounts;
                }
            }

            // Categories
            if (appState.categories && appState.categories.entities) {
                const categories = Object.values(appState.categories.entities);
                if (categories.length > 0) {
                    data[LocalStorageKey.GUEST_CATEGORIES] = categories;
                }
            }
